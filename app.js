...
// ================================================================
import { loadAllData, saveAll, saveLogs, saveActive, saveDebts, saveArchive, saveSalaries, saveEmployees } from "./sheets.js";
...
function updateEmployeeLists() {
    ...
    ["att-emp", "fin-emp", "att-filter", "fin-filter"].forEach(id => {
...
function renderFinance() {
    ...
    document.getElementById("fin-summary").innerHTML =
        `مساعده: <b>${totalActive.toLocaleString()}</b> | بدهی انتقالی: <b style="color:#f87171">${totalDebt.toLocaleString()}</b> | جمع کل: <b style="color:#818cf8">${total.toLocaleString()}</b> تومان`;
}
...
function openSettleDialog() {
    ...
    document.getElementById("settle-confirm").onclick = async () => {
        ...
        if (debtOut > 0) {
            STATE.debts.push({ id: uid(), date: toJalali(), name: filt, amount: debtOut, desc: `بدهی انتقالی از تسویه ${toJalali()}` });
        }
...
function renderArchive() {
    ...
    [...STATE.archive].reverse().forEach(r => {
        rows += `<tr onclick="showArchiveDetail('${r.id}')" style="cursor:pointer">
            ...
            <td>${r.debt_carried_out > 0 ? `<span class="badge red">${r.debt_carried_out.toLocaleString()}</span>` : `<span class="badge green">کامل</span>`}</td>
        </tr>`;
    });
...
function showArchiveDetail(id) {
    ...
    let html = `<div class="detail-info">
        ...
        ${rec.debt_carried_out > 0 ? `<span class="red">⚠️ بدهی انتقالی: ${rec.debt_carried_out.toLocaleString()}</span>` : `<span class="green">✅ تسویه کامل</span>`}
    </div><hr/>`;
...
function checkAdminPass() {
    ...
    return btoa(pass) === STATE.config.admin_pass ||
           STATE.config.admin_pass === btoa(pass);
}
...
async function changePassword() {
    const oldPass = prompt("رمز عبور فعلی:");
    if (!oldPass || btoa(oldPass) !== STATE.config.admin_pass) return showToast("رمز اشتباه است", "error");
    const newPass = prompt("رمز عبور جدید:");
